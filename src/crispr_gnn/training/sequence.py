...
def run_sequence_baselines(
    ...
    *,
    ...
    for model_type, model_name in variants:
        ...
        predictions.extend(_prediction_records(model_name, split_data, val_scores, test_scores))
...
def _result_row(
    *,
    ...
    return {
        ...
        "feature_columns": int(split_data["X_train"].shape[1] * split_data["X_train"].shape[2]),
        "threshold_policy": threshold_policy,
        "threshold": float(threshold),
        "notes": f"{model_name}; sequence-only {SEQUENCE_REPRESENTATION}; measured-only main split; experiment_id=18 excluded",
        **val_metrics,
        **test_metrics,
    }
