...
def threshold_report_rows(df: pd.DataFrame, cleavage: pd.Series) -> list[dict[str, Any]]:
    ...
    for name, config in EXPECTED_THRESHOLDS.items():
        ...
        positives = int(labels.sum())
        negatives = int((~labels).sum())
        measured_positive = int(labels[df["measured"] == 1].sum())
        measured_negative = int((~labels[df["measured"] == 1]).sum())
        putative_positive = int(labels[df["measured"] == 0].sum())
        putative_negative = int((~labels[df["measured"] == 0]).sum())
        ...
        rows.append(
            {
                ...
                "positives": positives,
                "negatives": negatives,
                "positive_rate": f"{positives / len(labels):.6f}",
                "imbalance": ratio_text(negatives, positives),
    ...
    policy_rows = [
        {
            ...
            "policy": "Label-ineligible until a documented policy is approved; do not silently impute.",
        },
        {
