...
REFERENCE_RUN_IDS = (
    ...
    "S7E_REF_GRAPH_A_GCN",
    "S7E_REF_GRAPH_C_GCN",
    "S7E_REF_FULL_GRAPH_C_GATV2",
    "S7E_REF_NO_CONTEXT_EDGE_GATV2",
)
HEADLINE_RUN_IDS = (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Sprint 7E Graph C GATv2 target-context subgroup ablations.")
    parser.add_argument("--config", default="configs/sweeps/sprint7e_target_context_subgroup_ablation.yaml")
...
def main() -> int:
    args = parse_args()
    run_sprint7e_target_context_subgroup_ablation(
...
def run_sprint7e_target_context_subgroup_ablation(
    *,
    ...
    output_dir = ROOT / str(config.get("outputs", {}).get("output_dir", "outputs/sprint7e"))
    ...
    for offset, run_spec in enumerate(run_specs, start=1):
        ...
        audit = _target_feature_mask_audit(materialized, run_config, run_id=run_id, run_spec=run_spec)
        ...
        audit.to_csv(run_dir / "target_feature_mask_audit.csv", index=False)
        ...
        manifest_runs.append(
            {
                ...
                "target_observation_mask_families": list(run_spec.get("target_observation_mask_families", [])),
                ...
                "target_feature_mask_audit_path": _relative(run_dir / "target_feature_mask_audit.csv"),
    ...
    results_path = output_dir / "target_context_subgroup_ablation.csv"
    predictions_path = diagnostics_dir / "target_context_subgroup_predictions.csv"
    history_path = diagnostics_dir / "target_context_subgroup_training_history.csv"
    attention_path = diagnostics_dir / "target_context_subgroup_attention_summary.csv"
    audit_path = diagnostics_dir / "target_context_subgroup_mask_audit.csv"
    ...
    manifest_path = output_dir / "target_context_subgroup_ablation_run_manifest.json"
    _write_run_manifest(
        manifest_path,
...
def _validate_base_config(config: Mapping[str, Any]) -> None:
    if config.get("sprint") != "sprint7e":
        raise ValueError("Sprint 7E runner requires sprint: sprint7e")
    if config.get("task") != "sprint7e_target_context_subgroup_ablation":
        raise ValueError("Sprint 7E runner requires task: sprint7e_target_context_subgroup_ablation")
    data = config.get("data", {})
    graph = config.get("graph", {})
    evaluation = config.get("evaluation", {})
    if data.get("label_scheme") != LABEL_SCHEME or data.get("split_id") != SPLIT_ID:
        raise ValueError("Sprint 7E must preserve Scheme A and sprint2_main_seed42")
    if graph.get("visibility_policy") != VISIBILITY_POLICY:
        raise ValueError("Sprint 7E must preserve strict-inductive visibility")
    if evaluation.get("protocol") != "headline_guide_level" or evaluation.get("no_test_tuning") is not True:
        raise ValueError("Sprint 7E must preserve headline guide-level no-test-tuning evaluation")
    features = config.get("features", {})
    if features.get("feature_set") != "S5F2_energy" or features.get("edge_feature_sets") != ["s5f2_energy"]:
        raise ValueError("Sprint 7E is frozen to S5F2_energy")
    training = config.get("training", {})
    if str(training.get("loss", "")).lower() != "weighted_bce":
        raise ValueError("Sprint 7E must keep weighted_bce")
    if training.get("loss_params", {}).get("pos_weight") != "auto":
        raise ValueError("Sprint 7E weighted BCE must keep pos_weight: auto")
    configured_ids = [str(run.get("id")) for run in config.get("runs", [])]
    if configured_ids != list(HEADLINE_RUN_IDS):
        raise ValueError("Sprint 7E headline run list/order drift")
    for run in config.get("runs", []):
        if run.get("architecture") != "gatv2":
            raise ValueError("Sprint 7E trains Graph C GATv2 ablations only")
        families = run.get("target_observation_mask_families")
        if not isinstance(families, list) or not families:
            raise ValueError(f"Sprint 7E run {run.get('id')} must declare target_observation_mask_families")
...
def _selected_run_specs(config: Mapping[str, Any], selected_run_ids: list[str] | None) -> list[dict[str, Any]]:
    ...
    for run_id in selected_run_ids:
        ...
        if run_id not in headline:
            ...
            raise ValueError(f"Unknown Sprint 7E run ID '{run_id}'. Allowed IDs: {allowed}")
...
def _config_for_run(
    ...
    *,
    ...
    run_config["model"]["target_observation_mask_families"] = list(run_spec["target_observation_mask_families"])
    ...
    run_config["sprint7e_run"] = {
        ...
        "target_observation_mask_families": list(run_spec["target_observation_mask_families"]),
...
def _validate_graph_c_artifacts(manifest: Mapping[str, Any]) -> None:
    if manifest.get("graph_name") != GRAPH_C:
        raise ValueError("Sprint 7E Graph C artifacts have wrong schema")
    feature_tables = manifest.get("feature_tables", {})
    if int(feature_tables.get("S5F2_energy", 0)) != 268:
        raise ValueError("Sprint 7E Graph C requires Sprint 5B S5F2_energy feature table")
    if int(feature_tables.get("target_observation_features", 0)) != sum(TARGET_CONTEXT_EXPECTED_COUNTS.values()):
        raise ValueError("Sprint 7E Graph C requires 212 target_observation_features")


def _target_feature_mask_audit(materialized: Any, config: Any, *, run_id: str, run_spec: Mapping[str, Any]) -> pd.DataFrame:
    ...
    for split in ["train", "val", "test"]:
        ...
        mask_indices = target_context_mask_indices(feature_names, config.target_observation_mask_families)
        mask_set = set(mask_indices)
        unmasked_indices = [index for index in range(len(feature_names)) if index not in mask_set]
        target_x = view["target_observation"].x
        masked_abs_before = float(target_x[:, list(mask_indices)].abs().sum())
        masked_x = target_x.clone()
        masked_x[:, list(mask_indices)] = 0.0
        ...
        rows.append(
            {
                ...
                "split": split,
                "graph_schema": GRAPH_C,
                "target_observation_mask_families": ",".join(config.target_observation_mask_families),
                "masked_column_count": int(len(mask_indices)),
                "masked_column_indexes": ",".join(str(index) for index in mask_indices),
                "masked_column_names": ",".join(feature_names[index] for index in mask_indices),
                "target_observation_feature_dim": int(target_x.shape[1]),
                "masked_feature_abs_sum_before_mask": masked_abs_before,
                "masked_feature_abs_sum_after_mask": float(masked_x[:, list(mask_indices)].abs().sum()),
                "unmasked_feature_abs_sum_after_mask": float(masked_x[:, unmasked_indices].abs().sum()),
...
def _reference_row(
    ...
    *,
    ...
    return {
        ...
        "notes": "Carry-forward matched-contract reference; not retrained in Sprint 7E runner.",
    }
...
def _insert_run_metadata(
    ...
    *,
    ...
    mask_families = ",".join(run_spec.get("target_observation_mask_families", []))
    payload = {
        "run_id": run_id,
        "run_order": order,
        "predeclared_run_id": run_spec["id"],
        "role": run_spec.get("role"),
        "controlled_variable": "target_observation_feature_family_mask",
        "drop_context_similarity_edges": True,
        "edge_blind_candidate_attention": False,
        "target_observation_mask_families": mask_families,
    }
    ...
    _insert_or_assign(predictions, 3, "target_observation_mask_families", mask_families)
    ...
    _insert_or_assign(history, 3, "target_observation_mask_families", mask_families)
...
def _write_diagnostics(
    ...
    threshold_path = diagnostics_dir / "target_context_subgroup_threshold_metrics.csv"
    ...
    deltas = results.copy()
    no_context_auprc = _reference_metric(deltas, "S7E_REF_NO_CONTEXT_EDGE_GATV2", "test_auprc")
    no_context_mcc = _reference_metric(deltas, "S7E_REF_NO_CONTEXT_EDGE_GATV2", "test_mcc")
    full_auprc = _reference_metric(deltas, "S7E_REF_FULL_GRAPH_C_GATV2", "test_auprc")
    graph_a_auprc = _reference_metric(deltas, "S7E_REF_GRAPH_A_GCN", "test_auprc")
    graph_c_gcn_auprc = _reference_metric(deltas, "S7E_REF_GRAPH_C_GCN", "test_auprc")
    xgb_auprc = _reference_metric(deltas, "S7E_REF_XGB_F4", "test_auprc")
    ...
    delta_path = diagnostics_dir / "target_context_subgroup_deltas.csv"
    deltas[
        [
            "run_id",
            "predeclared_run_id",
    ...
    attention_contract_path = diagnostics_dir / "target_context_subgroup_attention_contract_summary.csv"
    if attention.empty:
        pd.DataFrame(
    ...
    if not predictions.empty:
        ...
        per_guide_path = diagnostics_dir / "target_context_subgroup_per_guide_score_summary.csv"
        ...
        deciles_path = diagnostics_dir / "target_context_subgroup_score_deciles.csv"
...
def _write_figures(
    ...
    path = figures_dir / "target_context_subgroup_auprc_comparison.png"
    ...
    path = figures_dir / "target_context_subgroup_threshold_metrics.png"
    fig, ax = plt.subplots(figsize=(12, 4))
    ...
    if not predictions.empty:
        test_predictions = predictions.loc[predictions["split"] == "test"].copy()
        path = figures_dir / "target_context_subgroup_pr_curves.png"
        ...
        path = figures_dir / "target_context_subgroup_roc_curves.png"
        fig, ax = plt.subplots(figsize=(7, 5))
        for _run_id, group in test_predictions.groupby("run_id", sort=False):
        ...
        path = figures_dir / "target_context_subgroup_score_distributions.png"
    ...
    if not history.empty:
        path = figures_dir / "target_context_subgroup_training_curves.png"
    ...
    if not attention.empty:
        path = figures_dir / "target_context_subgroup_attention_by_edge_kind.png"
        fig, ax = plt.subplots(figsize=(9, 4))
        pivot = (
...
def _write_report(
    ...
    *,
    ...
    report = f"""# Sprint 7E Target-Observation Context Feature Subgroup Ablation Report
...
- Architecture policy: Graph C GATv2 only for newly trained runs; no new losses, samplers, encoders, graph schemas, or hyperparameter search.
- Primary ablation base: `context_similar_to` edges dropped; candidate S5F2 remains active in GATv2 attention and the final edge classifier.
- Controlled question: which source-defined direct `target_observation` feature family carries the Sprint 7D rare-negative behavior.
- Primary metric: AUPRC. MCC, specificity, TN/FP, and macro F1 are secondary operating-point diagnostics.
- Attention summaries and feature masks are interpretation-only model artifacts, not biological causal evidence.

## Result Summary

{_markdown_table(summary)}

## Canonical Mask Definitions

- `S7E_R1_mask_target_sequence`: masks the 115 target sequence one-hot columns.
- `S7E_R2_mask_experimental_epigenetic`: masks 6 experimental epigenetic scalar columns.
- `S7E_R3_mask_computed_nucleosome_aggregates`: masks 78 computed nucleosome aggregate columns.
- `S7E_R4_mask_computed_nucleosome_missingness`: masks 13 computed nucleosome missingness columns.
- `S7E_R5_mask_all_nonsequence_context`: masks the 97 nonsequence context columns.

## Interpretation Boundaries

- Compare 7E trained rows primarily against `S7E_REF_NO_CONTEXT_EDGE_GATV2`.
...
- No architecture, threshold, loss, sampler, feature-family, or rerun choice may be selected from test diagnostics.
...
def _write_graph_provenance(
    ...
    *,
    ...
    payload = {
        "provenance_type": "sprint7e_target_context_subgroup_artifacts_sha256",
        ...
        "run_specs": [
            {
                ...
                "role": run.get("role"),
                "target_observation_mask_families": list(run.get("target_observation_mask_families", [])),
                "drop_context_similarity_edges": True,
            }
...
def _write_run_manifest(
    ...
    *,
    ...
    payload = {
        "manifest_type": "sprint7e_target_context_subgroup_ablation_run_manifest",
        "generated_at_utc": datetime.datetime.now(datetime.UTC).isoformat(),
        "batch_id": batch_id,
        ...
        "target_feature_mask_audit_path": _relative(audit_path),
...
def _batch_id(config: Mapping[str, Any]) -> str:
    ...
    return f"sprint7e_target_context_subgroup_seed{int(config.get('seed', 42))}_{ts}"
...
def _validate_consolidated_run_ids(results: pd.DataFrame) -> None:
    if "run_id" not in results.columns:
        raise ValueError("Consolidated Sprint 7E results must include run_id")
    if results["run_id"].astype(str).duplicated().any():
        raise ValueError("Consolidated Sprint 7E results require unique run_id values")
