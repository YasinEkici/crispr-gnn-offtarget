...
Sprint 7/8A result: Graph C GATv2 target-context modelling is the strongest GNN direction under the frozen contract. Sprint 7F R3 remains the strongest carry-forward GNN AUPRC reference (test AUPRC 0.9849), while Sprint 8A selected R2 head-only FiLM (`S8A_R2_context_edge_film`) by validation AUPRC (test AUPRC 0.9828, MCC 0.5637). R2 is a validation-selected candidate, not a superiority claim. Slice 7 HP refinement was intentionally skipped to avoid post-result overtuning; robustness and superiority claims move to Sprint 9. Roadmap: Sprint 8B sequence-context encoder is the next model-improvement plan, and Sprint 9 is robustness (multi-seed, guide-level bootstrap CIs, paired-difference).
