...
This folder does not contain a final poster export yet. No final PDF, print-ready image, or final production source has been produced in this planning track. The existing LaTeX artifact is only a low-fidelity fit-check / working draft, not a final tool decision.
...
1. **Remove duplicate honesty caveat.**
   The exact honesty caveat currently appears inside `fig03_two_axis_results.svg` and again in `docs/poster/drafts/latex_fit_check/poster_fit_check.tex`.
   Keep it once, inside the results band:
...
2. **Create a real exported proof.**
   Export one full 70x100 cm proof from the chosen tool before print decisions.
...
- Start from `docs/poster/drafts/latex_fit_check/poster_fit_check.tex`.
- Treat it as a fit-check, not final code.
...
1. Choose the production tool.
2. Resolve the duplicated honesty caveat.
3. Select the final title.
4. Produce one full-size exported proof.
5. Run the checklist in `docs/poster/plan/poster_print_accessibility_review.md`.
6. Run the claim/number checklist in `docs/poster/plan/poster_claim_number_audit.md`.
7. Collect final source, export, assets, and references in this `docs/poster/final/` folder or another explicitly chosen final-delivery location.
